...
// ✅ تسجيل الانصراف (باستخدام UPSERT)
async function checkOutSecretary() {
  ...
  try {
    ...
    // ✅ استخدام upsert لتحديث check_out
    const { error: updateError } = await supabaseClient
      .from('secretary_attendance')
      .upsert(
        {
          id: existing.id, // مهم عشان يعمل update مش insert
          secretary_id: window.userId,
          attendance_date: today,
  ...
  async function renderSecretaryLog(){
    ...
    try {
      ...
      const { data, error } = await supabaseClient
        .from('secretary_attendance')
        .select('*')
        .eq('secretary_id', window.userId) // ✅ فلترة على السكرتير الحالي
