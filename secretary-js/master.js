...
document.getElementById("subscriptionForm").addEventListener("submit", async (e) => {
  ...
  try {
    ...
    // === جديد: رفع الصورة ===
    let avatarUrl = null;
    const avatarFileInput = document.getElementById("studentAvatar");
    if (avatarFileInput && avatarFileInput.files.length > 0) {
      const file = avatarFileInput.files[0];
      const fileName = `${Date.now()}_${file.name}`; // أو استخدم ID الطالب بعده

      // رفع الصورة إلى Supabase Storage
      const { data: uploadData, error: uploadError } = await supabaseClient
        .storage
        .from('picstudent') // اسم الـ bucket اللي إنتสรته
        .upload(fileName, file);

      if (uploadError) {
        console.error('خطأ في رفع الصورة:', uploadError.message);
        showStatus('خطأ في رفع الصورة', 'error');
        // ممكن تكمل بدون صورة أو توقف العملية
        // return; // لو عايز توقف
      } else {
        ...
        // console.log('رابط الصورة:', avatarUrl); // للتأكد
      }
    }
    // === نهاية رفع الصورة ===

    // إضافة avatar_url للبيانات إذا كانت موجودة
    if (avatarUrl) {
       studentData.avatar_url = avatarUrl;
    }
    ...
    const paymentData = {
      ...
      status: paid >= price ? "paid" : "partial"
    };
