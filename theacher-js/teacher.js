    ...
    // زر تشغيل الكاميرا - مع منع التكرار
    const startQrScanButton = document.getElementById("startQrScan");
    if (startQrScanButton) {
    // ✅ إزالة أي مستمعين سابقين للحدث عشان ما يحصلش تكرار
    const newStartQrScanButton = startQrScanButton.cloneNode(true);
    startQrScanButton.parentNode.replaceChild(newStartQrScanButton, startQrScanButton);

    newStartQrScanButton.addEventListener("click", () => {
        const qrReader = document.getElementById("qr-reader");
        if (qrReader) qrReader.style.display = "block";
        const stopBtn = document.getElementById("stopQrScan");
        if (stopBtn) stopBtn.style.display = "inline-block";
        newStartQrScanButton.style.display = "none";
        // إعداد الماسح
    ...
    console.error("زر startQrScan غير موجود في الصفحة.");
    }
